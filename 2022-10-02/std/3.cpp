...
void Exgcd(int a, int b, Long &x, Long &y, int &d) {
  if (b == 0) {
    x = 1;
    y = 0;
    d = a;
...
inline void ModDivTo(int &a, int b) {
  a = (Long)a * ModInv(b) % Mod;
}

int n, m, m1, origin[Maxn];
int set[Maxn];
vector<Long> add[Maxn];
int mul[Maxn + 1], cnt_mul;

struct cmp {
  bool operator () (int i, int j) const {
    return (LLong)add[i][add[i].size() - 2] * add[j].back()
      < (LLong)add[i].back() * add[j][add[j].size() - 2];
  }
};
...
int main(void) {
  ...
  while (m1--) {
    ...
    if (mul_1_up == -1 || (mul_2 != -1 && (LLong)mul_2 * mul_1_down > mul_1_up)) {
      if (mul_2 == -1) {
        break;
      }
      // Apply 2
